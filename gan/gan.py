...
class GAN():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        if os.path.exists('X.pickle'):
           X,Y= self.build_dataset(self.new_path)           
        else:
            #Load saved datasets
           pickle_in = open('X.pickle','rb')
           X = pickle.load(pickle_in)
...
if __name__ == '__main__':
    #initialize gan class with path of images to build a dataset
    gan = GAN(r"/cats-vs-dogs/")
    #-- build the dataset
    gan.train(epochs=2000, batch_size=128, sample_interval=200)
